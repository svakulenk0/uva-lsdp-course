...
from Datasets.src.OneHotEncoder1 import OneHotEncoder
import os
...
class Model:
    ...
    def read_data2(self):
        CMU = pd.read_csv("../MovieSummaries/movie.metadata.tsv", sep = '\t', header = None)
        movies_new, genre_counts = gen.preprocess(CMU)
        movies_new['clean_plot'] = movies_new['plot'].apply(lambda x: gen.clean_text(x))
        y = movies_new['genre_new']
        X = movies_new['clean_plot']
        xtrain, xval, ytrain, yval = train_test_split(X, y, test_size=0.2)
        self.train_contents = np.array(xtrain)
        self.train_labels = np.array([x[0] for x in ytrain])
        self.test_contents = np.array(xval)
        self.test_labels = np.array([x[0] for x in yval])
